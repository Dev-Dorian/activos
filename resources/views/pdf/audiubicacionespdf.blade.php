...
<head>
    ...
    <title>Reporte de Auditoria de Ubicaciones</title>
    <style>
        .centro {
        	text-align: center;
            line-height: 0.005;
...
<body>
    ...
    Centro Diurno para la Persona Adulta Mayor de El Tejar</h4>
          ...
          <h4>
          <span class="izquierda">Reporte de Auditoria de Ubicaciones</span>
    ...
    <tbody>
        @foreach ($ubicaciones_auditoria as $a)
            <tr>
            <td>{{$a->nombreUbicacion}}</td>
            <td>{{$a->descripcionUbicacion}}</td>
            <td>{{$a->created_at}}</td>
            <td>{{$a->updated_at}}</td>
            <td>{{$a->condicionUbicacion?'Activo':'Desactivado'}}</td>                      
